...
function getUserMobile($uid){
    //根据uid
    $tmp=db("eastlady_user")->where(array('uid'=>$uid))->column("mobile");
//    p($tmp);exit;
    return $tmp[0];
}
...
function replace_space($str){
    $tmpstr=htmlentities($str);
    $ret = preg_replace("/(\s+)/", ' ',$tmpstr);
    return $ret;
...
{
    //通过数据表获取 省市
//        id找到市 array('id'=>$user['id'])
    $map['id'] = $id;
    $citys = db("region")->field("name")->where($map)->find();
    $msg = "查找成功!";
    return $citys;
//    exit(parent::showJson($citys, 1, $msg));
}

//把  省份 城市
//$province_id = $up_data['province_id'];
//$city_id  = $up_data['city_id'];
//$province=db("region")->field('name')->where("id",$province_id)->find();
//$city=db("region")->field('name')->where("id",$city_id)->find();